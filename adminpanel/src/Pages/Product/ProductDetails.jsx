...
export default function ProductDetails() {
  ...
  let getParentData = () => {
    axios.get(`${apiBaseUrl}product/view-parentCat`)
      .then((res) => {
        return res.data;
      })
      .then((finalRes) => {
        if (finalRes.status == 1) {
          setParentData(finalRes.data);
        }
      })
  };
  useEffect(() => {
  ...
  let getSubCatData = () => {
    axios.get(`${apiBaseUrl}product/view-subCat`)
      .then((res) => {
        return res.data;
      })
      .then((finalRes) => {
        if (finalRes.status == 1) {
          setSubCatData(finalRes.data);
        }
      })
  }
  useEffect(()=>{
  ...
  let getSizeData = () => {
    axios.get(`${apiBaseUrl}product/view-size`)
      .then((res) => {
        return res.data;
      })
      .then((finalRes) => {
  ...
  let getColorData = () => {
    axios.get(`${apiBaseUrl}product/view-color`)
      .then((res) => {
        return res.data;
      })
      .then((finalRes) => {
                  ...
                  >
                    <option selected>--Select Size--</option>
                    {
                      ...
                      SizeData.map((item,index)=>{
                        return(
                          <option value={item._id}>{item.sizeName}</option>
                        )
                      })
                      :
                      ''
